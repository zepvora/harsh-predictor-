...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_for_digits"):
        ...
        response = (
            ...
            f"<code>[{bar}]</code>\n\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🔢 Digit Sum:  {result['digit_sum']}\n"
            f"📈 BIG Score:  {result['big_score']}/8\n"
            f"📉 SMALL Score: {result['small_score']}/8\n\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🎟️ Credits Left: {remaining}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"⚠️ <i>Sirf entertainment ke liye</i>\n"
            f"💬 Loss recovery: @{DM_USERNAME}"
        )
        ...
        if remaining <= 2:
            refer_link = f"https://t.me/{BOT_USERNAME}?start={user.id}"
            kb.insert(1, [InlineKeyboardButton("🔗 Refer & Earn Credits", url=refer_link)])
        ...
        if not_joined:
            ...
            udata = get_user(user.id)
            credits = udata["credits"] if udata else 0
            await update.message.reply_text(f"🎟️ Tumhare Credits: <b>{credits}</b>\n\n👇 Menu use karo:", parse_mode="HTML", reply_markup=main_keyboard(credits))
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"  /checkuser &lt;id&gt; — User detail\n"
        f"  /addcredits &lt;id&gt; &lt;amount&gt;\n"
        f"  /removecredits &lt;id&gt; &lt;amount&gt;\n"
        f"  /ban &lt;id&gt; — Ban user\n"
...
async def cmd_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in recent:
        ...
        lines.append(f"{status} <b>{fname}</b> ({uname_str})\n   ID: <code>{uid}</code> | Credits: {credits} | Preds: {preds}\n")
...
async def cmd_checkuser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"👤 <b>User Info</b>\n\n"
        f"Name: <b>{udata['first_name']}</b>\n"
        f"Username: @{udata['username'] or 'None'}\n"
        f"ID: <code>{uid}</code>\n"
        f"Status: {status}\n"
        f"Credits: <b>{udata['credits']}</b>\n"
        f"Refers: <b>{udata['refer_count']}</b>\n"
        f"Predictions: <b>{udata['predictions_count']}</b>\n"
        f"Joined: {str(udata['joined_at'])[:16] if udata['joined_at'] else 'N/A'}\n\n"
        f"{'🔓 /unban ' + str(uid) if udata['banned'] else '🔒 /ban ' + str(uid)}",
        parse_mode="HTML")
...
async def cmd_addcredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(uid,
            f"🎉 <b>Credits Mile!</b>\n\n🎟️ Admin ne <b>{amount} credits</b> add kiye!\n💰 Ab tumhare credits: <b>{new_credits}</b>",
            parse_mode="HTML")
    except Exception:
        pass
    await update.message.reply_text(f"✅ Done!\n👤 User: <code>{uid}</code>\n🎟️ Added: <b>{amount}</b>\n💰 Total: <b>{new_credits}</b>", parse_mode="HTML")

async def cmd_removecredits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        return
    if len(context.args) != 2:
